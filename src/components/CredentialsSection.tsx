  ...
  return (
    ...
    >
      <motion.div
        ...
        className="w-full h-full rounded-xl border border-slate-800 bg-black p-6 shadow-[0_10px_30px_rgba(16,185,129,0.03)] flex flex-col justify-between overflow-hidden relative group hover:border-emerald-500/30"
      >
        ...
        <div style={{ transform: 'translateZ(40px)' }} className="space-y-2">
          ...
          <h3 className="text-sm font-bold text-white tracking-wide leading-snug font-sans group-hover:text-emerald-400 transition-colors">
            {credential.title}
          </h3>
          <p className="text-[10px] text-slate-400 font-mono">
            ISSUER: <span className="text-slate-300 font-semibold">{credential.issuer}</span>
          </p>
        </div>

        {/* Footer block */}
        <div style={{ transform: 'translateZ(30px)' }} className="flex justify-between items-center text-[10px] font-mono border-t border-slate-800 pt-3">
          <div>
            <span className="text-slate-500 block uppercase">Auth Date</span>
            <span className="text-emerald-400 font-bold">{credential.date}</span>
          </div>

          <div className="text-right">
            <span className="text-slate-500 block uppercase">Trust Status</span>
  ...
  return (
    <div id="academic-ledger-verification-portal" className="w-full space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-emerald-500/10 pb-6">
        <div>
          <div className="inline-flex items-center gap-1.5 text-xs font-mono font-bold tracking-wider uppercase mb-3 bg-[#0d121b] text-emerald-400 px-3 py-1 rounded-md border border-emerald-500/20 shadow-md">
        ...
        <div className="flex p-1 bg-[#0d121b] rounded-lg border border-slate-800 shadow-sm">
          <button
            type="button"
        ...
        {activeTab === 'deck' ? (
          ...
          >
            ...
            <div className="flex flex-col items-center gap-3">
              ...
              <div className="w-32 h-[2px] bg-[#0d121b] border border-slate-800 rounded-full overflow-hidden relative">
                <motion.div
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
              <div className="md:col-span-4 relative">
                ...
                <input
                  ...
                  className="w-full bg-[#0d121b] border border-slate-800 rounded-lg py-2.5 pl-10 pr-4 text-xs font-mono text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-colors shadow-sm"
                />
              </div>
                ...
                {categories.map((cat) => (
                  <button
                    ...
                    className={`px-3 py-1.5 text-[10px] font-mono tracking-wider border rounded-md transition-all cursor-pointer min-h-[44px] flex items-center justify-center ${
                      activeCategory === cat
                        ? 'border-emerald-500 bg-emerald-500 text-black font-bold shadow-sm shadow-emerald-500/10'
                        : 'border-slate-800 bg-[#0d121b] text-slate-400 hover:border-emerald-500/30 hover:text-emerald-400 shadow-sm'
                    }`}
                  >
                    {cat}
                  </button>
            ...
            <div className="overflow-x-auto rounded-lg border border-slate-800 bg-black shadow-md">
              <table className="w-full border-collapse text-left text-xs font-mono">
                <thead>
                  <tr className="bg-[#070b12]/80 border-b border-slate-800 text-slate-400">
                ...
                <tbody className="divide-y divide-slate-800/60">
                  {filteredCredentials.length > 0 ? (
                    filteredCredentials.map((c) => (
                      <tr 
                        key={c.id}
                        className="hover:bg-[#0d121b]/50 transition-colors text-slate-300 group"
                      >
                        <td className="p-4">
                        ...
                        <td className="p-4 text-slate-400">{c.issuer}</td>
                        <td className="p-4 text-emerald-400 font-semibold">{c.date}</td>
                        <td className="p-4">
                          <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded bg-emerald-950/20 border border-emerald-500/20 text-emerald-400 font-semibold text-[9px]">
